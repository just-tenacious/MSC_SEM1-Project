...
const AdminUsers = ({ type }) => {
  ...
  const handleVerify = async (id) => {
    try {
      await axios.post("http://localhost:3001/users/update-status", { _id: id, status: 1 });
      setUsers(prev =>
        prev.map(u => (u._id === id ? { ...u, acc_status: 1 } : u))
      );
          ...
          {displayedRows.length ? (
            displayedRows.map((u, idx) => (
              <tr key={u._id} style={trStyle}>
                ...
                <td style={tdStyle}>
                  {u.acc_status === 0 ? (
                    <button style={verifyBtn} onClick={() => handleVerify(u._id)}>Verify</button>
                  ) : (
                    <>
                      <button
...
const verifyBtn = { padding: "4px 8px", background: "#28a745", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", marginRight: "5px" };
