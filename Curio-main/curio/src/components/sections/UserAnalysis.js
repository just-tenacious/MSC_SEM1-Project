...
const analysisData = [
  ...
  //   url: "/analysis-clothing",
  // },
  {
    type: "clothing",
    title: "Style Preference Analysis",
    desc: "Analyze your height and body build to find clothes that fit perfectly and enhance your natural proportions.",
    iconClass: "fas fa-tshirt",
    url: "/analysis-clothing",
  },
];
...
const UserAnalysis = () => {
  ...
  return (
    <section className="section" id="analysis">
      <h3>Analysis</h3>
      <p>
        Use our tools to understand your unique features and discover the styles
        that suit you best.
      </p>
      ...
      <div className="analysis-container scroll-animate">
        {analysisData.map((item, idx) => (
          <div className="analysis-card" key={idx}>
            ...
            <h4>{item.title}</h4>
            <p>{item.desc}</p>
            <button className="btn-small" onClick={() => handleClick(item.url)}>
              Analyze
            </button>
          </div>
        ))}
      ...
      <p className="analysis-note">
        Tip: Combining your face, body, and style analyses provides a full
        fashion profile for optimal outfit recommendations.
      </p>
    </section>
  );
};
